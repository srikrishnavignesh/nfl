...
class NFLDataset(Dataset):
    ...
    def __getitem__(self, indx):
        if indx in self.cache:
            return self.cache[indx]
        else:
            item = self._load_item(indx)
            self.cache[indx] = item
            return item
...
class SpatialTemporalLayer(Module):
    def __init__(self, d_model, nhead, spatial_encoder_layers, dropout, in_features):
        ...
        self.spatial_encoder = TransformerEncoder(encoder_layer=transformer_encoder_layer,enable_nested_tensor=False, 
                                                  num_layers=spatial_encoder_layers)
        
        self.temporal_encoder = GRU(input_size=d_model, hidden_size=d_model, num_layers=1, batch_first=True, dropout=dropout)
        self.td_output_projection = Linear(in_features = d_model, out_features = features_to_predict)

        self.cross_attn = MultiheadAttention(embed_dim=d_model, num_heads=nhead, batch_first=True, dropout=dropout)
...
def train(model, train_data, test_data, std_scaler, model_type = ModelType.DX_MODEL):
    runner = Runner(model, train_data, test_data, std_scaler, model_type, lr=2.5e-5, wd=1e-4)
    runner.run()
...
def predict(dx_model, dy_model, test_data, std_scaler):
    ...
    df.to_csv('cross_attn_with_mlp_test_data_results.csv', index=False)
